...
def getData(netcdf2d,obj,variable,_odimensions=None):
  ...
  for dim in _dimensions:
    ...
    if dim=="ntime" and obj['user_time']: # Does it need to be interpolated?
      if obj['inter.temporal']['temporal']=="closest":data=timeSeriesClosest(obj["_datetime"],obj["datetime"],data)
...
def getDimData(netcdf2d,obj,dimensions):
  ...
  for dim in dimensions:
    ...
    if dim=="node":
      ...
      data['snode']={"data":None,"subdata":{
        "sx":{"data":obj['sx'],"meta":netcdf2d.getMetaByVariable('sx')},
        "sy":{"data":obj['sy'],"meta":netcdf2d.getMetaByVariable('sy')}
      }}
    elif dim=="time":
